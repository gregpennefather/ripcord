...
export class VideoDAL {
  ...
  lastRefresh?: number;
  refreshNeeded = () =>
    !this.lastRefresh || Date.now() - this.lastRefresh > 60 * 1000;

  addVideo = async (fileName: string): Promise<void> => {
    if (!this.hasVideo(fileName)) {
      const videoFile = await this.buildVideoFileInfo(fileName);
      this.videos.push(videoFile);
    }
  };

  addVideoFileInfo = async (fileName: string): Promise<void> => {
    const videoFileInfo = await readInfoFile(this.videosPath, fileName);
  ...
  private async refresh(): Promise<void> {
    const files = await getFileList(this.videosPath);
    const infoFiles = files.filter((fName) => extname(fName) == '.json');
  ...
  private async buildVideoFileInfo(fileName: string): Promise<VideoFileInfo> {
    ...
    const infoFilePath = join(this.videosPath, baseName + '.json');
    return new Promise<VideoFileInfo>(async (resolve, reject) => {
      const videoFilePath = join(this.videosPath, fileName);
      const stats = await getFileStats(videoFilePath);
      const subtitles = await getSubtitles(this.videosPath, baseName);
      const video: VideoFileInfo = {
        uuid: randomUUID(),
        fileName,
      ...
      writeFile(infoFilePath, JSON.stringify(video), (err) =>
...
const readInfoFile = async (
  videoPath: string,
  fileName: string,
): Promise<VideoFileInfo> =>
  new Promise((resolve, reject) => {
    const path = join(videoPath, fileName);
    readFile(path, (err, data) => {
      if (err) reject(err);
      const info: VideoFileInfo = JSON.parse(data.toString());
      resolve(info);
    });
  });

const getFileList = async (videoPath: string): Promise<string[]> =>
  new Promise((resolve, reject) => {
...
const getFileStats = async (path: string): Promise<Stats> =>
  new Promise((resolve, reject) =>
...
const getSubtitles = async (
  ...
  return new Promise(async (resolve, reject) => {
    const reg = RegExp(`${baseName}(\.[a-z][a-z])(\.srt|\.vtt)`);
    readdir(videoPath, async (err, files) => {
      if (err) reject(err);

      const subFiles: SubtitleFile[] = [];
      await files
        .map((f) => {
          const regMatches = reg.exec(f);
        ...
        .forEach(async ([fileName, locale, ext]) => {
          if (ext === 'vtt') {
            const vtt: SubtitleFile = {
              lang: locale,
              path: join(videoPath, `${fileName}`),
            };
            subFiles.push(vtt);
          } else {
            const vttPath = join(videoPath, `${baseName}.${locale}.vtt`);
            if (!existsSync(vttPath)) {
              const filePath = await createVtt(
                videoPath,
                fileName,
                baseName,
                locale,
...
async function createVtt(
  ...
  return new Promise<string>((resolve, reject) => {
    readFile(join(videoPath, fileName), (err, data) => {
      if (err) reject(err);
      const vttFilePath = join(videoPath, `${baseName}.${locale}.vtt`);
